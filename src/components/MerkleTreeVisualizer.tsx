...
export default function MerkleTreeVisualizer({
  ...
  const [activeTab, setActiveTab] = useState<
    "visualization" | "merklepath" | "calculation"
  >("visualization");
  ...
  const hashBinary = useCallback(
    async (leftHash: string, rightHash: string): Promise<string> => {
  ...
  const buildMerkleTree = useCallback(
    async (dataItems: string[]): Promise<TreeNode | null> => {
  ...
  const calculateMerklePath = useCallback(
    async (txids: string[], targetIndex: number): Promise<string[]> => {
  ...
  const findMerklePath = useCallback(
    async (tree: TreeNode, targetLeafId: string): Promise<MerklePathStep[]> => {
      // Find the target leaf and its position
      const findLeafNode = (
        node: TreeNode
      ): { node: TreeNode | null; index: number } => {
      ...
      try {
        const siblingHashes = await calculateMerklePath(
          leafHashes,
          targetIndex
        );
  ...
  const getPathNodes = useCallback(
    (tree: TreeNode, targetLeafId: string): Set<string> => {
  ...
  const calculateMerkleProofSteps = useCallback(
    async (
      ...
      for (let i = 0; i < path.length; i++) {
        ...
        if (step.isTarget) {
          ...
          steps.push({
            ...
            rightHash,
            result: resultHash,
            description: `Position ${currentIndex} (${
              isEven ? "even" : "odd"
            }): ${isEven ? "current+sibling" : "sibling+current"}`,
          });
  ...
  useEffect(() => {
    if (merklePath.length > 0 && selectedLeaf && tree) {
      ...
      if (leafNode) {
        calculateMerkleProofSteps(leafNode.hash, merklePath, selectedLeaf)
          .then((steps) => {
            setProofSteps(steps);
            setCalculatingSteps(false);
          })
  ...
  const renderNode = (
    ...
    const isRoot =
      node.level ===
      (tree ? Math.max(...getAllNodes(tree).map((n) => n.level)) : 0);
  ...
  const getAllNodes = (node: TreeNode): TreeNode[] => {
    ...
    if (node.right && node.right !== node.left)
      nodes.push(...getAllNodes(node.right));
    return nodes;
  };
  ...
  const renderTree = () => {
    ...
    const maxNodesInLevel = Math.max(
      ...nodesByLevel.map((level) => level.length)
    );
    ...
    const effectiveSpacing = showPathOnly
      ? minNodeSpacing + 40
      : minNodeSpacing;
      ...
      >
        <svg width={svgWidth} height={svgHeight} className="bg-black block">
          ...
          {(() => {
            ...
            const rootX =
              rootLevel.length > 0 ? 50 + rootSpacing * (0 + 1) : svgWidth / 2;
            const rootY = startY;
            ...
            return levelNodes.map((node, index) => {
              ...
              if (node.left && (!showPathOnly || pathNodes.has(node.left.id))) {
                ...
                if (leftIndex !== -1) {
                  ...
                  const leftY =
                    startY + (maxLevel - node.left.level) * levelHeight;
              ...
              if (
                ...
                (!showPathOnly || pathNodes.has(node.right.id))
              ) {
                ...
                if (rightIndex !== -1) {
                  ...
                  const rightY =
                    startY + (maxLevel - node.right.level) * levelHeight;
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-xl text-[#d1d5db] max-w-3xl mx-auto">
          Build and visualise Merkle trees with full hash display and
                ...
                {tree && (
                  ...
                  >
                    ...
                    <div className="p-3 bg-gray-900 rounded border">
                      <p className="font-mono text-sm text-blue-400 break-all leading-relaxed">
                        {tree.hash}
                      </p>
                    </div>
                    {showPathOnly && selectedLeaf && (
                      <div className="mt-3 p-2 bg-blue-900/20 border border-blue-500/30 rounded text-sm text-blue-300">
                        🎯 Showing merkle path for selected transaction:{" "}
                        {selectedLeaf.replace("leaf-", "")}
                      </div>
                    )}
                  </div>
                )}
                ...
                {merklePath.length > 0 && (
                  <div className="mt-6 p-4 bg-gray-800 rounded-lg border border-gray-700">
                    <h3 className="text-lg font-semibold text-white mb-4">
                      🛤️ Merkle Path{" "}
                      {selectedLeaf &&
                        `for Transaction ${selectedLeaf.replace("leaf-", "")}`}
                    </h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                    ...
                    <p className="text-sm text-blue-300 mb-4">
                      The final calculated hash should match the merkle root to
                      prove the transaction is included in the block.
                    </p>
                    {tree && (
                      <div>
